...
func (u *UserPostgresRepo) FollowUser(follow model.User_followers) error {
	...
	u.db.Model(&user).Where("username", follow.Follower).Update("following", gorm.Expr("following + 1"))

	//update followee - followers count +1
	u.db.Model(&user).Where("username", follow.Followee).Update("followers", gorm.Expr("followers + 1"))
